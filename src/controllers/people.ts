...
export const getPerson: RequestHandler = async (req, res): Promise<any> => {
...
export const addPerson: RequestHandler = async (req, res): Promise<any> => {
  const { id_event, id_group } = req.params;
  ...
  if (!body.success) return res.json({ error: "Dados inválidos" });
...
export const updatePerson: RequestHandler = async (req, res): Promise<any> => {
  const { id, id_event, id_group } = req.params;
...
export const deletedPerson: RequestHandler = async (req, res): Promise<any> => {
  const { id, id_event, id_group } = req.params;
...
export const searchPerson: RequestHandler = async (req, res): Promise<any> => {
  const { id_event } = req.params;
  const searchPersonSchema = z.object({
    cpf: z.string().transform((val) => val.replace(/\.|-/gm, "")),
  });
  const query = searchPersonSchema.safeParse(req.query);
  if (!query.success) return res.json({ error: "Dados inválidos" });
